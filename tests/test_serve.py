...
def test_drift_flags_year_before_training_range():
    ...
    drifted = serve.check_drift(1700, 6, 52.0, 21.0, "Poland")
    assert "year" in drifted


def test_no_drift_for_typical_input():
    drifted = serve.check_drift(2010, 6, 52.0, 21.0, "Poland")
    assert drifted == []


def test_unknown_country_is_flagged():
    drifted = serve.check_drift(2000, 6, 0.0, 0.0, "Atlantyda")
    assert "Country" in drifted
